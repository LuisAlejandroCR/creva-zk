...
`docs/DESPLIEGUE-LACE.md`. With the address but no key the circuit aborts on
the signature check, which reads on screen as "no yet" about an identity that
was in fact valid — so the port refuses to join rather than pay for a proof
that cannot clear.
